...
import { getRangeBuilderRanking } from "@/lib/supabase/rangeSpots";
import { ACCENT } from "@/lib/poker/constants";
import { RangeBuilderIcon } from "@/components/ToolIcons";

const MIN_ATTEMPTS = 5;

export default function RangeBuilderRankingPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setRanking(await getRangeBuilderRanking(MIN_ATTEMPTS));
      } finally {
        setLoading(false);
      }
    })();
    ...
    <div style={{ minHeight: "100vh", padding: 20, maxWidth: 520, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontSize: 19, fontWeight: 700, letterSpacing: -0.3 }}>Range Builder</span>
      ...
      <div style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 14, padding: 18 }}>
        ...
        <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 12 }}>
          % de similarité moyen, tous spots confondus, minimum {MIN_ATTEMPTS} tentatives
        </div>
            ...
            {ranking.map((p, i) => (
              ...
              >
                ...
                <span style={{ fontFamily: "var(--font-ibm-plex-mono), monospace", color: ACCENT }}>
                  {Number(p.avg_accuracy).toFixed(0)}% · {p.total_attempts} tentatives
                </span>
              </div>
            ))}
          </div>
